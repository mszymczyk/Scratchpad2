...
SVD_mats svd( float3x3 A )
{
	...
	float3 D;
	float3x3 V;
	GetSVD3D( A, svd_result.U, D, V );

	svd_result.Sigma = ( float3x3 )0;
	svd_result.Sigma[0][0] = D.x;
	svd_result.Sigma[1][1] = D.y;
	svd_result.Sigma[2][2] = D.z;

	svd_result.V = transpose( V );
