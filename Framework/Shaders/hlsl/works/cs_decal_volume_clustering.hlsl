...
#define DECAL_VOLUME_CLUSTER_THREADS_PER_GROUP		128
...
void DecalTilingPass1( uint3 cellThreadID : SV_GroupThreadID, uint3 cellID : SV_GroupID )
{
	...
	//ci.cellIndex = DecalVolume_GetCellFlatIndex( uint3( cellID.xy, 0 ), uint3( cellCountA.xy, 1 ) );
	//ci.parentCellIndex = DecalVolume_GetCellFlatIndex( uint3( cellID.xy / 2, 0 ), uint3( cellCountA.xy / 2, 1 ) );
