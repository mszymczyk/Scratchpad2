...
int HlslShader::_ParseHlsl( const u8* buf, size_t bufSize, const char* filename )
{
	...
	for ( u32 iprog = 0; iprog < nUniquePrograms; ++iprog )
	{
		...
		up.stage = static_cast<fxlib::e_ProgramType>( stage );
		...
		if ( up.stage == fxlib::eProgramType_vertexShader )
		{
		...
		else if ( up.stage == fxlib::eProgramType_pixelShader )
		{
		...
		else if ( up.stage == fxlib::eProgramType_geometryShader )
		{
		...
		else if ( up.stage == fxlib::eProgramType_computeShader )
		{
	...
	for ( u32 ipass = 0; ipass < nPasses; ++ipass )
	{
		...
		for ( u32 icomb = 0; icomb < flatSize; ++icomb )
		{
			...
			{
				u32 entryId_[static_cast<u32>( fxlib::eProgramType_count )];
			} upi;

			fr.readChunk( &upi, sizeof( upi ) );

			u32 vsIndex = upi.entryId_[(size_t)fxlib::eProgramType_vertexShader];
			u32 psIndex = upi.entryId_[(size_t)fxlib::eProgramType_pixelShader];
			u32 gsIndex = upi.entryId_[(size_t)fxlib::eProgramType_geometryShader];
			u32 csIndex = upi.entryId_[(size_t)fxlib::eProgramType_computeShader];

			if ( vsIndex != 0xffffffff )
			{
				const HlslUniqueProgram& up = uniquePrograms[vsIndex];

				SPAD_ASSERT( up.stage == fxlib::eProgramType_vertexShader );
				pass.vs_ = up.vs;
				pass.vs_->AddRef();
			...
			{
				SPAD_ASSERT( uniquePrograms[psIndex].stage == fxlib::eProgramType_pixelShader );
			...
			{
				SPAD_ASSERT( uniquePrograms[gsIndex].stage == fxlib::eProgramType_geometryShader );
			...
			{
				SPAD_ASSERT( uniquePrograms[csIndex].stage == fxlib::eProgramType_computeShader );
