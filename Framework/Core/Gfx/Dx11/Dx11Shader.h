...
#include <Tools/FxCompiler/FxCompilerLib/FxCompilerLib.h>
...
public:
	...
	void setVS( ID3D11DeviceContext* context ) const { SPAD_ASSERT( vs_ ); context->VSSetShader( vs_, nullptr, 0 ); }
	void setPS( ID3D11DeviceContext* context ) const { SPAD_ASSERT( ps_ ); context->PSSetShader( ps_, nullptr, 0 ); }
	void setCS( ID3D11DeviceContext* context ) const { SPAD_ASSERT( cs_ ); context->CSSetShader( cs_, nullptr, 0 ); }
};
...
struct HlslUniqueProgram
{
	...
	fxlib::e_ProgramType stage = fxlib::eProgramType_count;
};
